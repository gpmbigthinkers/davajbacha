...
`;
    ...
    class ShaderPass {
      ...
      constructor(props: { material?: SimProps extends never ? never : { vertexShader: string; fragmentShader: string; uniforms: Record<string, { value: unknown }> }; output: THREE.WebGLRenderTarget | null }) {
        this.props = props;
    ...
    class Advection extends ShaderPass {
      ...
      update(opts: { dt: number; isBounce: boolean; BFECC: boolean }) {
        (this.uniforms.dt as { value: number }).value = opts.dt;
        if (this.line) this.line.visible = opts.isBounce;
        (this.uniforms.isBFECC as { value: boolean }).value = opts.BFECC;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: { mouse_force: number; cursor_size: number; cellScale: THREE.Vector2 }) {
        ...
        if (!(this.mouse instanceof THREE.Mesh)) return;
        const mat = this.mouse.material as THREE.RawShaderMaterial;
        const u = mat.uniforms;
        (u.force!.value as THREE.Vector2).set(forceX, forceY);
        (u.center!.value as THREE.Vector2).set(centerX, centerY);
        (u.scale!.value as THREE.Vector2).set(props.cursor_size, props.cursor_size);
        super.update();
      }
    }

    class Viscous extends ShaderPass {
      private output0: THREE.WebGLRenderTarget;
      private output1: THREE.WebGLRenderTarget;

      constructor(simProps: SimProps) {
        const out0RT = simProps.dst_!;
        const out1RT = simProps.dst;
        super({
          material: {
            vertexShader: face_vert,
            ...
            uniforms: {
              ...
              velocity: { value: simProps.src.texture },
              velocity_new: { value: out0RT.texture },
      ...
      update(opts: { viscous: number; iterations: number; dt: number }) {
        const u = this.props.material!.uniforms;
        (u.v as { value: number }).value = opts.viscous;
        ...
        for (let i = 0; i < opts.iterations; i++) {
          ...
          (u.velocity_new as { value: THREE.Texture }).value = fbo_in.texture;
          this.props.output = fbo_out;
          (u.dt as { value: number }).value = opts.dt;
          super.update();
        }
        return fbo_out;
      }
    ...
    class Divergence extends ShaderPass {
      ...
      update(opts: { vel: THREE.WebGLRenderTarget }) {
        (this.uniforms.velocity as { value: THREE.Texture }).value = opts.vel.texture;
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      constructor(simProps: SimProps) {
        const out0 = simProps.dst_!;
        const out1 = simProps.dst;
        super({
          material: {
            vertexShader: face_vert,
      ...
      update(opts: { iterations: number }) {
        ...
        for (let i = 0; i < opts.iterations; i++) {
          ...
          (this.uniforms.pressure as { value: THREE.Texture }).value = p_in.texture;
    ...
    class Pressure extends ShaderPass {
      ...
      update(opts: { vel: THREE.WebGLRenderTarget; pressure: THREE.WebGLRenderTarget }) {
        (this.uniforms.velocity as { value: THREE.Texture }).value = opts.vel.texture;
        (this.uniforms.pressure as { value: THREE.Texture }).value = opts.pressure.texture;
        super.update();
      }
    }
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      if (webgl.autoDriver['mouse']) {
        const m = webgl.autoDriver['mouse'] as MouseClass;
        m.autoIntensity = autoIntensity;
        m.takeoverDuration = takeoverDuration;
      }
    }
    if (resolution !== prevRes) {
      sim.resize();
